...
class NeuralSDF(torch.nn.Module):
    ...
    def set_normal_epsilon(self, current_iter=None, warm_up_end=5000):
        if self.cfg_sdf.encoding.coarse2fine.enabled:
            if self.cfg_sdf.encoding.coarse2fine.smooth_sched:
                eps_lv = max(
                    (current_iter - warm_up_end * 2)
                    / self.cfg_sdf.encoding.coarse2fine.step,
                    0,
                )
                eps_lv = min(eps_lv, self.cfg_sdf.encoding.levels)
                epsilon_res = self.r_min * self.growth_rate**eps_lv + 1
            else:
        ...
        self.normal_eps = (
            2.0 / epsilon_res
        )  # TODO: may not need, factor of 2 as bounding region is doubled

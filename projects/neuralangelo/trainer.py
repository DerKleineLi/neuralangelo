...
class Trainer(BaseTrainer):
    ...
    def _compute_loss(self, data, mode=None):
        if mode == "train":
            ...
            self.losses["render"] = (
                self.criteria["render"](data["rgb"], data["image_sampled"]) * 3
            )  # FIXME:sumRGB?!
            self.metrics["psnr"] = (
                -10 * torch_F.mse_loss(data["rgb"], data["image_sampled"]).log10()
            )
            if "eikonal" in self.weights.keys():
                self.losses["eikonal"] = eikonal_loss(
                    data["gradients"], outside=data["outside"]
                )
            if "curvature" in self.weights:
                self.losses["curvature"] = curvature_loss(
                    data["hessians"], outside=data["outside"]
                )
        else:
            # Compute loss on the entire image.
            self.losses["render"] = self.criteria["render"](
                data["rgb_map"], data["image"]
            )
            self.metrics["psnr"] = (
        ...
        if "curvature" in self.weights:
            if current_iteration <= self.warm_up_end:
                self.weights["curvature"] = (
                    current_iteration / self.warm_up_end * init_weight
                )
            else:
        ...
        if self.cfg.model.object.sdf.encoding.coarse2fine.enabled:
            ...
            if self.cfg_gradient.mode == "numerical":
                model.neural_sdf.set_normal_epsilon(
                    current_iteration, warm_up_end=self.cfg.optim.sched.warm_up_end
                )
                if self.cfg.model.object.sdf.encoding.coarse2fine.smooth_sched:
                    cont_lv = max(
                        (current_iteration - self.cfg.optim.sched.warm_up_end)
                        / self.cfg.model.object.sdf.encoding.coarse2fine.step,
                        0,
                    )
                    cont_lv = min(cont_lv, self.cfg.model.object.sdf.encoding.levels)
                    decay_factor = model.neural_sdf.growth_rate**cont_lv
                else:
                    decay_factor = (
                        model.neural_sdf.growth_rate**model.neural_sdf.anneal_levels
                    )
                self.get_curvature_weight(
                    current_iteration,
                    self.cfg.trainer.loss_weight.curvature,
                    decay_factor,
                )
        elif self.cfg_gradient.mode == "numerical":
            model.neural_sdf.set_normal_epsilon(
                current_iteration, warm_up_end=cfg.optim.sched.warm_up_end
            )
    ...
    def log_wandb_scalars(self, data, mode=None):
        ...
        if self.cfg.model.object.sdf.encoding.coarse2fine.enabled:
            scalars[
                f"{mode}/active_levels"
            ] = self.model.module.neural_sdf.active_levels
            ...
            images.update(
                {
                    ...
                    f"{mode}/vis/normal": wandb_image(
                        data["normal_map"], from_range=(-1, 1)
                    ),
                    f"{mode}/vis/inv_depth": wandb_image(
                        1
                        / (data["depth_map"] + 1e-8)
                        * self.cfg.trainer.depth_vis_scale
                    ),
    ...
    def train(self, cfg, data_loader, single_gpu=False, profile=False, show_pbar=False):
        self.progress = self.model_module.progress = (
            self.current_iteration / self.cfg.max_iter
        )

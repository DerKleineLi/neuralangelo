...
class Trainer(BaseTrainer):
    ...
    def get_curvature_weight(self, current_iteration, init_weight, decay_factor):
        if "curvature" in self.weights:
            weight = (
                min(current_iteration / self.warm_up_end, 1.0)
                if self.warm_up_end > 0
                else 1.0
            ) * init_weight
    ...
    def _start_of_iteration(self, data, current_iteration):
        ...
        if self.cfg.model.object.sdf.encoding.coarse2fine.enabled:
            ...
            if self.cfg_gradient.mode == "numerical":
                ...
                decay_factor = (
                    model.neural_sdf.growth_rate**model.neural_sdf.add_levels
                )  # TODO: verify?
    ...
    def log_wandb_scalars(self, data, mode=None):
        ...
        if mode == "train" and self.cfg.optim.log_gradient_contribugion:
            for loss_name, contribution in self.gradient_contribution.items():
                scalars[f"{mode}/gradient_contribution/{loss_name}"] = contribution
